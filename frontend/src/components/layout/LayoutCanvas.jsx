...
// Interior buttons & controls
import InteriorCameraControls from "../van/VanInteriorCameraControls";
import SpotLightCom from "../van/VanSpotsLight";
...
const LayoutCanvas = () => {
  ...
  const camPros = [targetPos[0], targetPos[1], targetPos[2] + CAMERA_OFFSET];
  ...
  useEffect(() => {
    ...
    const layoutPartsData = allModels.data.data.filter((model) => {
      const modelLabel = model?.label?.trim().toLowerCase();
      return allowedLabels.includes(modelLabel);
    });
  ...
  return (
    ...
    >
      ...
      <Canvas
        camera={{ position: [0, 2, 10], fov: 35, near: 0.1, far: 1000 }}
        style={{ width: "100%", height: "100%", background: "#2F2F2F" }}
        dpr={[1, 1.5]}
        shadows
      >
        {isIntView ? (
